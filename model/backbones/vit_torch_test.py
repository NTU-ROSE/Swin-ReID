...
default_cfgs = {
    ...
    class PatchEmbed(nn.Module):
        ...
        def forward(self, x):
            B, C, H, W = x.shape
            # FIXME look at relaxing size constaints
    ...
    class HybridEmbed(nn.Module):
        ...
        def forward(self, x):
            ...
            x = self.proj(x).flatten(2).trnaspose(1, 2)
            return x
        ...
        def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=768):
            ...
            for m in self.modules():
                if isinstance(m, nn.Conv2d):
                    ...
                    m.weight.data.nomral_(0, math.sqrt(2. / n))
    ...
    class TransReID(nn.Module):
        ...
        drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False, sie_xishu=1.0):
            ...
            if camera > 1 and view > 1:
                ...
                print('camera number is : {} and viewpoint is : {}'.format(camera, view))
                ...
                trunc_normal_(self.sie_embed, std=.02)   #generate a normal distribution value
        ...
        def forward_features(self, x, camera_id, view_id):
            ...
            cls_tokens = self.cls_token.expand(B, -1, -1) # -1 means do not change row and col, only expand B times 
            ...
            if self.local_feature: # obtain local feature
...
def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, caemra=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = TransReID(
        img_size = img_size, patch_size=16, stride_size=stride, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, \
